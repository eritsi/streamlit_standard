    ...
    * **ATTENTION:**
    ...
    if st.button('inference'):
        if df_true is None:
            ...
            # st.write(conv_df)
            df_inf = pd.concat([conv_df, pd.DataFrame(df_inf, columns=[
                               'sales']), X_inference[[selected_classification_col]]], axis=1)
    ...
    if st.button('Graph'):
        ...
        ML_df = pd.merge(
            df_true, ML_df[[ML_df.columns[0], selected_classification_col]].drop_duplicates())
        ...
        for c in sorted(selected_clusters):
            pivot_df = pivot_df_for_dengram(
                ML_df[ML_df[selected_classification_col] == c].iloc[:, 0:4])
            # st.write(pivot_df.T)
