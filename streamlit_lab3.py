...
def app():
    ...
    * **ATTENTION:**
    ...
    if 'input_df' not in st.session_state:
        ...
        # sorted_features = sorted(list(set(df.columns) -
        # set(df.columns[3:4]))) # - set("shifted_count")
        ...
        if st.button('Create Model'):
            if mdl == 'Light GBM':
                ...
                # ②学習期間のdfを切り出す
                df_train = pd.merge(
                    df_modeling, df_time[:-to_period], how='inner')
                # 学習対象を選ばれしクラスタのみにする
                ...
                df_inference = pd.merge(
                    df_modeling,
                    df_time.tail(to_period),
                    how='inner')[selected_features]
